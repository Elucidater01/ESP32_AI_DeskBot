...
esp_err_t baidu_chatbot_send_request(char *system_content, char *user_content)
{
    ...
    // asprintf(&url_str, BAIDU_LLM_URL, API_KEY);
    ...
    // esp_http_client_set_header(baidu_erniebot_http_handle, "Authorization", "Bearer "API_KEY);
    ...
    esp_http_client_set_header(baidu_erniebot_http_handle, "Authorization", auth_header);
    esp_http_client_set_header(baidu_erniebot_http_handle, "Content-Type", "application/json");
    ...
    asprintf(&post_data,
        ...
        "{\"role\":\"user\",\"content\":\"%s\"}"
        "]}",
        "ernie-4.5-turbo-128k", system_content, user_content ? user_content : "");
...
esp_err_t baidu_chatbot_recv_response(char *response_data, size_t output_size)
{
    ...
    if (choices && cJSON_GetArraySize(choices) > 0) {
        ...
        if (first_choice) {
            ...
            if (content && cJSON_IsString(content)) {
                snprintf(response_data, output_size, "%s", content->valuestring);
    ...
    if (ret != ESP_OK) {
        ESP_LOGE(TAG, "Failed to parse content");
    }
...
esp_err_t baidu_asr_send_audio(const int16_t *audio, size_t len, size_t total_len)
{
    ...
    if (g_baidu_asr_http_handle == NULL) {
        ...
        char *url_str = NULL;
        int ret = asprintf(&url_str, BAIDUBCE_STT_URL, CONFIG_BAIDU_AUDIO_ACCESS_TOKEN);
